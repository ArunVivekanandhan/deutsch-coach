...
(function () {
  ...
  function satzbauItems() {
    ...
    D.grammar.filter(t => TOPICS.includes(t.id)).forEach(t => t.sentences.forEach(s => {
      ...
      out.push({ mode: 'en', q: s.en, ta: s.ta || '', answer: de, alts: [], con, grp, lv: s.lv || t.level, why: RULE[grp], topic: t.title });
    }));
    return out;
  }
  ...
  function joinItems() {
    ...
    return J.map(j => {
      ...
      return { mode: 'join', a: j.a, b: j.b, q: `${j.a} + ${j.b}`, answer: j.de, alts: j.alts || [], con: j.c, grp,
               lv: ADUSO.includes(j.c.toLowerCase()) ? 'A1' : 'A2', why: (j.why ? j.why + ' — ' : '') + RULE[grp], topic: 'Crashkurs' };
    });
  }
  ...
  function draw() {
    ...
    $('kbBar').innerHTML = bar();
    if (!K.list.length) { box.innerHTML = '<div class="kb-empty">Für diese Auswahl gibt es keine Sätze — wähle ein anderes Level oder „Alle“.</div>'; return; }
    if (K.k >= K.list.length) {
      box.innerHTML = `<div style="text-align:center;"><div class="kb-score">${K.right} / ${K.total}</div><div>Sätze richtig gebaut</div>
    ...
    const prompt = it.mode === 'join'
      ? `<div class="kb-join"><div>1️⃣ ${esc(it.a)}</div><div>2️⃣ ${esc(it.b)}</div></div><div class="kb-en">Verbinde beide Sätze zu <b>einem</b> Satz.</div>`
    ...
    box.innerHTML = `<div class="kb-badge">Satz ${K.k + 1} / ${K.list.length} · ✅ ${K.right} · ${esc(it.lv)}</div>
      ${prompt}
      <div class="kb-hint" style="border-color:${col}">mit <b>${esc(it.con)}</b> · ${ic} ${esc(gl)}</div>
  ...
  function result(ok, given, shown, note) {
    ...
    $('kbRes').innerHTML = `<div class="kb-fb ${ok ? 'ok' : 'no'}">${ok ? '✅ Richtig!' : shown ? '👁 Lösung:' : '❌ Nicht ganz. Richtig:'}
        ...
        <div class="kb-why">💡 ${esc(it.why)}</div></div>${expl}
